...
class Nilai_Tugas extends CI_Controller{
	function __construct(){
		parent::__construct();
		$this->load->database();
		$this->load->library(array("pagination","SessNilai"));
	...
	function index(){
		$this->sessnilai->resetSessNilai("tugas");
		if(!$this->session->has_userdata("user") || !$this->session->has_userdata("hak_akses"))redirect("admin/login");
			...
			$arr["siswa_6771.kd_kelas"]=$this->get_userdata("nilai_tugas","kd_kelas");
			$arr["siswa_6771.nis"]=$this->get_userdata("nilai_tugas","nis_cok");
			$arr["nilai_tugas_6771.kd_mapel"]=$this->get_userdata("nilai_tugas","kd_mapel");
			$arr["nilai_tugas_6771.semester"]=$this->get_userdata("nilai_tugas","semester");
			$arr["nilai_tugas_6771.tahun_ajaran"]=$this->get_userdata("nilai_tugas","tahun_ajaran");
			...
			if(isset($getrequest["q"]) && !empty($getrequest["q"])){
				...
				unset($this->session->userdata["nilai_tugas"]["kd_kelas"]);
				unset($this->session->userdata["nilai_tugas"]["nis_cok"]);
				unset($this->session->userdata["nilai_tugas"]["kd_mapel"]);
				unset($this->session->userdata["nilai_tugas"]["semester"]);
				unset($this->session->userdata["nilai_tugas"]["tahun_ajaran"]);
			}else{
			...
			$data["nis_value"]=$this->get_userdata("nilai_tugas","nis_cok")!=null?$this->get_userdata("nilai_tugas","nis_cok"):'0';
			$data["kd_kelas_value"]=$this->get_userdata("nilai_tugas","kd_kelas")!=null?$this->get_userdata("nilai_tugas","kd_kelas"):'0';
			$data["kd_mapel_value"]=$this->get_userdata("nilai_tugas","kd_mapel")!=null?$this->get_userdata("nilai_tugas","kd_mapel"):'0';
			$data["semester_value"]=$this->get_userdata("nilai_tugas","semester")!=null?$this->get_userdata("nilai_tugas","semester"):'0';
			$data["tahun_ajaran_value"]=$this->get_userdata("nilai_tugas","tahun_ajaran")!=null?$this->get_userdata("nilai_tugas","tahun_ajaran"):'0';
	...
	function gantiPaging(){
		if(!$this->session->has_userdata("user") || !$this->session->has_userdata("hak_akses"))redirect("admin/login");
			...
			if(isset($post["kd_kelas"]) && isset($post["nis"]) && isset($post["kd_mapel"]) && isset($post["semester"]) && isset($post["tahun_ajaran"])){
				$this->session->userdata["nilai_tugas"]["kd_kelas"]=$post["kd_kelas"];
				$this->session->userdata["nilai_tugas"]["nis_cok"]=$post["nis"];
				$this->session->userdata["nilai_tugas"]["kd_mapel"]=$post["kd_mapel"];
				$this->session->userdata["nilai_tugas"]["semester"]=$post["semester"];
				$this->session->userdata["nilai_tugas"]["tahun_ajaran"]=$post["tahun_ajaran"];
