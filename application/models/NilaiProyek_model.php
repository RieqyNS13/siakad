<?php 
class NilaiProyek_model extends CI_Model{
	...
	function total($arr=null){
		...
		if($arr!=null){
			foreach($arr as $key=>$mbuh){
				if(!is_null($mbuh)){
					if($key=="nilai_proyek_6771.kd_mapel" && $mbuh==="00"){
		...
		$this->db->select("*");
		$this->db->from("nilai_proyek_6771");
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_proyek_6771.nis");
		$this->db->group_by(array("nilai_proyek_6771.nis","nilai_proyek_6771.kd_mapel","nilai_proyek_6771.semester","nilai_proyek_6771.tahun_ajaran"));
		return $this->db->get()->num_rows();
	}
	function cariData($q,$start,$limit){
		$this->db->select("siswa_6771.*,mapel_6771.*,nilai_proyek_6771.*,guru_6771.nama as nama_guru,round(avg(nilai_proyek_6771.nilai),2) as rata2,count(nilai) as jumlah,concat(kelas_6771.prefix_kelas,' ',jurusan_6771.nama_jurusan,' ',kelas_6771.nomor_kelas) as kelas,concat(nilai_proyek_6771.semester,' (',IF(nilai_proyek_6771.semester=1,'Ganjil','Genap'),')') as semester2");
		$this->db->from("nilai_proyek_6771");
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_proyek_6771.nis");
		$this->db->join("mapel_6771","mapel_6771.kode_mapel=nilai_proyek_6771.kd_mapel");
		...
		$this->db->join("guru_6771","guru_6771.kode_guru=nilai_proyek_6771.kd_guru_penilai");
		...
		$this->db->or_having("nilai_proyek_6771.semester like '%".$q2."%'");
		$this->db->or_having("nilai_proyek_6771.tahun_ajaran like '%".$q2."%'");
		$this->db->or_having("nilai_proyek_6771.nilai like '%".$q2."%'");
	...
	function jumlahcariData($q){
		$this->db->select("*,round(avg(nilai_proyek_6771.nilai),2) as rata2,count(nilai) as jumlah,concat(kelas_6771.prefix_kelas,' ',jurusan_6771.nama_jurusan,' ',kelas_6771.nomor_kelas) as kelas,concat(nilai_proyek_6771.semester,' (',IF(nilai_proyek_6771.semester=1,'Ganjil','Genap'),')') as semester2");
		$this->db->from("nilai_proyek_6771");
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_proyek_6771.nis");
		$this->db->join("mapel_6771","mapel_6771.kode_mapel=nilai_proyek_6771.kd_mapel");
		$this->db->join("kelas_6771","kelas_6771.id_kelas=siswa_6771.kd_kelas");
	...
	function getNilaiByNis(){
		...
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_proyek_6771.nis");
		$this->db->join("mapel_6771","mapel_6771.kode_mapel=nilai_proyek_6771.kd_mapel");
		$this->db->where("nilai_proyek_6771.nis",$this->input->post("nis"));
		$this->db->order_by("nilai_proyek_6771.id","asc");
		$get=$this->db->get();
	...
	function editData(){
		...
		$get=$this->db->get_where("nilai_proyek_6771",$where);
		$result=$get->result();
		$i=0;
		foreach($result as $key=>$mbuh){
			if(!in_array($mbuh->id,$post["id"])){
				$this->db->delete("nilai_proyek_6771",array("id"=>$mbuh->id));
			}else{
				...
				$this->db->update("nilai_proyek_6771",array("nilai"=>$nilai));
				unset($nilaitmp[$cari]);
			}
		}
		foreach($nilaitmp as $cok){
			$nilai=(int)$cok;
			if($nilai<0 || $nilai>100)return array("error"=>"Nilai harus di antara 0-100");
			$this->db->insert("nilai_proyek_6771",array_merge($where,array("nilai"=>$cok,"kd_guru_penilai"=>$this->session->userdata("kode_guru"))));
		}
		$this->db->where($where);
		$edit=$this->db->update("nilai_proyek_6771",array("semester"=>$post["semester"],"tahun_ajaran"=>$post["tahun_ajaran"]));
	...
	function inputData(){
		...
		foreach($nilais as $key=>$nilai){
			...
			$this->db->insert("nilai_proyek_6771", $post);
		}
	...
	function getNilai(){
		...
		$data['nilai_proyek_6771.nis']=$this->input->get("nis");
		$data['nilai_proyek_6771.kd_mapel']=$this->input->get("kd_mapel");
		$data['nilai_proyek_6771.semester']=$this->input->get("semester");
		$data['nilai_proyek_6771.tahun_ajaran']=$this->input->get("tahun_ajaran");
		foreach($data as $mbuh)if(empty($mbuh))return array("error"=>"Parameter gak lengkap");
		$this->db->select("nilai_proyek_6771.*,siswa_6771.*,mapel_6771.*,guru_6771.nama as namaguru,concat(kelas_6771.prefix_kelas,' ',jurusan_6771.nama_jurusan,' ',kelas_6771.nomor_kelas) as kelas,concat(nilai_proyek_6771.semester,' (',IF(nilai_proyek_6771.semester=1,'Ganjil','Genap'),')') as semester2");
		$this->db->from("nilai_proyek_6771");
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_proyek_6771.nis");
		$this->db->join("kelas_6771","kelas_6771.id_kelas=siswa_6771.kd_kelas");
		$this->db->join("jurusan_6771","jurusan_6771.id_jurusan=kelas_6771.kd_jurusan");
	...
	function hapusData(){
		...
		$hapus=$this->db->delete("nilai_proyek_6771",$post);
