<?php 
class NilaiPortofolio_model extends CI_Model{
	...
	function total($arr=null){
		...
		if($arr!=null){
			foreach($arr as $key=>$mbuh){
				if(!is_null($mbuh)){
					if($key=="nilai_portofolio_6771.kd_mapel" && $mbuh==="00"){
		...
		$this->db->select("*");
		$this->db->from("nilai_portofolio_6771");
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_portofolio_6771.nis");
		$this->db->group_by(array("nilai_portofolio_6771.nis","nilai_portofolio_6771.kd_mapel","nilai_portofolio_6771.semester","nilai_portofolio_6771.tahun_ajaran"));
		return $this->db->get()->num_rows();
	}
	function cariData($q,$start,$limit){
		$this->db->select("siswa_6771.*,mapel_6771.*,nilai_portofolio_6771.*,guru_6771.nama as nama_guru,round(avg(nilai_portofolio_6771.nilai),2) as rata2,count(nilai) as jumlah,concat(kelas_6771.prefix_kelas,' ',jurusan_6771.nama_jurusan,' ',kelas_6771.nomor_kelas) as kelas,concat(nilai_portofolio_6771.semester,' (',IF(nilai_portofolio_6771.semester=1,'Ganjil','Genap'),')') as semester2");
		$this->db->from("nilai_portofolio_6771");
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_portofolio_6771.nis");
		$this->db->join("mapel_6771","mapel_6771.kode_mapel=nilai_portofolio_6771.kd_mapel");
		...
		$this->db->join("guru_6771","guru_6771.kode_guru=nilai_portofolio_6771.kd_guru_penilai");
		...
		$this->db->or_having("nilai_portofolio_6771.semester like '%".$q2."%'");
		$this->db->or_having("nilai_portofolio_6771.tahun_ajaran like '%".$q2."%'");
		$this->db->or_having("nilai_portofolio_6771.nilai like '%".$q2."%'");
	...
	function jumlahcariData($q){
		$this->db->select("*,round(avg(nilai_portofolio_6771.nilai),2) as rata2,count(nilai) as jumlah,concat(kelas_6771.prefix_kelas,' ',jurusan_6771.nama_jurusan,' ',kelas_6771.nomor_kelas) as kelas,concat(nilai_portofolio_6771.semester,' (',IF(nilai_portofolio_6771.semester=1,'Ganjil','Genap'),')') as semester2");
		$this->db->from("nilai_portofolio_6771");
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_portofolio_6771.nis");
		$this->db->join("mapel_6771","mapel_6771.kode_mapel=nilai_portofolio_6771.kd_mapel");
		$this->db->join("kelas_6771","kelas_6771.id_kelas=siswa_6771.kd_kelas");
	...
	function getNilaiByNis(){
		...
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_portofolio_6771.nis");
		$this->db->join("mapel_6771","mapel_6771.kode_mapel=nilai_portofolio_6771.kd_mapel");
		$this->db->where("nilai_portofolio_6771.nis",$this->input->post("nis"));
		$this->db->order_by("nilai_portofolio_6771.id","asc");
		$get=$this->db->get();
		if($get->num_rows()>=1)return $get->result();
		else return array("error"=>"Data nilai tidak ditemukan");
	}
	function cekJumlahNilai($arr){
		$this->db->select("*");
		$this->db->from("nilai_portofolio_6771");
		$this->db->where($arr);
		$get=$this->db->get();
		if($get->num_rows()>=5)return false;
		else return true;
	}
	function editData(){
		...
		$get=$this->db->get_where("nilai_portofolio_6771",$where);
		$result=$get->result();
		$i=0;
		foreach($result as $key=>$mbuh){
			if(!in_array($mbuh->id,$post["id"])){
				$this->db->delete("nilai_portofolio_6771",array("id"=>$mbuh->id));
			}else{
				...
				$this->db->update("nilai_portofolio_6771",array("nilai"=>$nilai));
				unset($nilaitmp[$cari]);
			}
		}
		foreach($nilaitmp as $cok){
			$nilai=(int)$cok;
			if($nilai<0 || $nilai>100)return array("error"=>"Nilai harus di antara 0-100");
			$this->db->insert("nilai_portofolio_6771",array_merge($where,array("nilai"=>$cok,"kd_guru_penilai"=>$this->session->userdata("kode_guru"))));
		}
		$this->db->where($where);
		$edit=$this->db->update("nilai_portofolio_6771",array("semester"=>$post["semester"],"tahun_ajaran"=>$post["tahun_ajaran"]));
		if($edit)return array("sukses"=>"Sukses edit nilai proyek");
		else{
			$error=$this->db->error();
	...
	function inputData(){
		...
		$cek=$this->cekJumlahNilai(array("nis"=>$post["nis"],"kd_mapel"=>$post["kd_mapel"],"semester"=>$post["semester"],"tahun_ajaran"=>$post["tahun_ajaran"]));
		if(!$cek)return array("error"=>"Jumlah nilai praktek sudah mencapai maximal, tidak bisa menambah lagi");
		foreach($nilais as $key=>$nilai){
			...
			$this->db->insert("nilai_portofolio_6771", $post);
		}
	...
	function getSiswaByKelas($all="true"){
		...
		if($kd_kelas=='0' || $kd_kelas==null)$get=$this->db->get("siswa_6771")->result();
		else $get=$this->db->get_where("siswa_6771",array("kd_kelas"=>$kd_kelas))->result();
		$asu='';
		if(count($get)>0 && $all=="true")$asu.='<option value="0">Semua Siswa</option>';
		foreach($get as $data){
			$asu.="<option value=\"".$data->nis."\">".$data->nis." | ".$data->nama."</option>";
		}
		return $asu;
	...
	function getNilai(){
		...
		$data['nilai_portofolio_6771.nis']=$this->input->get("nis");
		$data['nilai_portofolio_6771.kd_mapel']=$this->input->get("kd_mapel");
		$data['nilai_portofolio_6771.semester']=$this->input->get("semester");
		$data['nilai_portofolio_6771.tahun_ajaran']=$this->input->get("tahun_ajaran");
		foreach($data as $mbuh)if(empty($mbuh))return array("error"=>"Parameter gak lengkap");
		$this->db->select("nilai_portofolio_6771.*,siswa_6771.*,mapel_6771.*,guru_6771.nama as namaguru,concat(kelas_6771.prefix_kelas,' ',jurusan_6771.nama_jurusan,' ',kelas_6771.nomor_kelas) as kelas,concat(nilai_portofolio_6771.semester,' (',IF(nilai_portofolio_6771.semester=1,'Ganjil','Genap'),')') as semester2");
		$this->db->from("nilai_portofolio_6771");
		$this->db->join("siswa_6771","siswa_6771.nis=nilai_portofolio_6771.nis");
		$this->db->join("kelas_6771","kelas_6771.id_kelas=siswa_6771.kd_kelas");
		$this->db->join("jurusan_6771","jurusan_6771.id_jurusan=kelas_6771.kd_jurusan");
		...
		}else return array("jumlah"=>$jumlah,"msg"=>"Tidak ada record nilai. Nilai dimulai dari proyek 1");
	}
	function hapusData(){
		$post=$this->input->post();
		if(count($post)==0)return array("error"=>"Tidak ada parameter");
		$hapus=$this->db->delete("nilai_portofolio_6771",$post);
