...
function detailNilai(id){
	$.ajax({
		url:'<?php echo site_url('admin/nilai_sikap/getNilaisikap'); ?>',
		type:'get',
		data:{id:id},
		dataType:'json',
		beforeSend:function(){
		...
		$("#dataObservasi").html(data.nilai_observasi);
		$("#dataDiri").html(data.nilai_diri);
		$("#dataAntarTeman").html(data.nilai_antarteman);
		$("#dataJurnal").html(data.nilai_jurnal);
		$("#dataRata2").html(data.rata2);
		var konversi;
		switch(data.rata2){
			case "4":konversi='SB';break;
			case "3":konversi='B';break;
			case "2":konversi='C';break;
			case "1":konversi='K';break;
			default:konversi='Tidak Ada';break;
		}
		$("#dataKonversiNilai").html(konversi);
		$("#detailJudul").html("Detail Nilai sikap");
...
function editNilai(id){
	...
	$.ajax({
		...
		$("#modalEditNilai [name='nilai_observasi']").val(data.nilai_observasi);		
		$("#modalEditNilai [name='nilai_diri']").val(data.nilai_diri);		
		$("#modalEditNilai [name='nilai_antarteman']").val(data.nilai_antarteman);		
		$("#modalEditNilai [name='nilai_jurnal']").val(data.nilai_jurnal);		
...
$(function(){
	...
	$("#konfirmHapus").click(function(){
		...
		$.ajax({
			url:'<?php echo site_url("admin/nilai_sikap/hapusNilaisikap"); ?>',
			type:'post',
			data:data,
			dataType:'json'
		}).done(function(data){
	...
	$("#modalTambahNilai [name='kd_kelas'],#modalSetting [name='kd_kelas']").change(function(e){
		...
		$.ajax({
			url:'<?php echo site_url('admin/nilai_sikap/getSiswaByKelas'); ?>',
			type:'get',
	...
	$("#modalLog").on("hidden.bs.modal",function(e){
		if($("body").data("isRefresh")==true)window.location.reload();
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-table"></i> Nilai Sikap
                            </li>
                        </ol>
                    </div>
                </div>
				...
				<div class="col-lg-6">
				 <form class="form-inline" id="gantiPaging" method="post" action="<?php echo site_url("admin/nilai_sikap/gantiPaging"); ?>">
			...
			<form id="frmCari" method="get" action="<?php echo site_url("admin/nilai_sikap/index"); ?>">
			 <div class="col-lg-4">
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
								   ...
								   foreach($dataNilai as $data){
									   echo "<tr><td>$a</td><td>".$data->nis."</td><td>".$data->nama."</td><td>".$data->kelas."</td><td>".$data->nama_mapel."</td><td>".$data->semester2."</td><td>".$data->tahun_ajaran."</td><td>".$data->nilai_observasi."</td><td>".$data->nilai_diri."</td><td>".$data->nilai_antarteman."</td><td>".$data->nilai_jurnal."</td><td>";
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalTambahNilai">
  <div class="modal-dialog">
	...
	<form id="frmTambahNilai" method="post" action="<?php echo site_url("admin/nilai_sikap/tambahNilaisikap"); ?>">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Input Nilai Sikap</h4>
      </div>
...
<div class="form-group">
    <label class="lblNilai">Nilai Observasi</label>
	<div class="row">
	<div class="col-sm-6" >
		<input type="text" class="form-control" name="nilai_observasi" placeholder="Masukkan Nilai (1-4)">
	</div>
	</div>
</div>
<div class="form-group">
    <label class="lblNilai">Nilai Diri</label>
	<div class="row">
	<div class="col-sm-6" >
		<input type="text" class="form-control" name="nilai_diri" placeholder="Masukkan Nilai (1-4)">
	</div>
	</div>
</div>
<div class="form-group">
    <label class="lblNilai">Nilai Antar Teman</label>
	<div class="row">
	<div class="col-sm-6" >
		<input type="text" class="form-control" name="nilai_antarteman" placeholder="Masukkan Nilai (1-4)">
	</div>
	</div>
</div>
<div class="form-group">
    <label class="lblNilai">Nilai Jurnal</label>
	<div class="row">
	<div class="col-sm-6" >
		<input type="text" class="form-control" name="nilai_jurnal" placeholder="Masukkan Nilai (1-4)">
	</div>
	</div>
</div>
</p>
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalEditNilai">
  <div class="modal-dialog">
    <div class="modal-content">
	<form id="frmeditNilai" method="post" action="<?php echo site_url("admin/nilai_sikap/editNilaisikap"); ?>">
	<input type="hidden" name="idEdit" value="">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Edit Nilai Sikap</h4>
      </div>
      <div class="modal-body">
        <p>
  <div class="form-group">
    <label>NIS | Nama Siswa</label>
<input type="text" name="nis" class="form-control ora" disabled="disabled" value="">
  </div>
  <div class="form-group">
    <label>Mapel</label>
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalSetting">
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
	<form method="post" action="<?php echo site_url("admin/nilai_sikap/gantiPaging"); ?>" >
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalDetail">
 ...
 <tr><td>Nilai Observasi</td><td>:</td><td id="dataObservasi">rieqyns13</td></tr>
 <tr><td>Nilai Diri</td><td>:</td><td id="dataDiri">rieqyns13</td></tr>
 <tr><td>Nilai Antar Teman</td><td>:</td><td id="dataAntarTeman">rieqyns13</td></tr>
 <tr><td>Nilai Jurnal</td><td>:</td><td id="dataJurnal">rieqyns13</td></tr>
 <tr><td>Nilai Rata-rata</td><td>:</td><td id="dataRata2">rieqyns13</td></tr>
 <tr><td>Konversi Nilai</td><td>:</td><td id="dataKonversiNilai">rieqyns13</td></tr>
