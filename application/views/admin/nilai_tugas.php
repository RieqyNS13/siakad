...
function detailNilai(nis,kd_mapel,semester,tahun_ajaran){
	$.ajax({
		url:'<?php echo site_url('admin/nilai_tugas/getNilaiTugas'); ?>',
		type:'get',
		...
		if(dataArr.jumlah>0){
			...
			for(var i=0; i<dataArr.jumlah; i++){
				njir+='<tr class="nilaitugas"><td>Nilai Tugas '+(i+1)+'</td><td>:</td><td>'+dataArr.msg[i].nilai+'</td></tr>';
...
function editNilai(nis,kd_mapel,semester,tahun_ajaran){
	...
	$.ajax({
		...
		var asu='<div class="form-group"><label class="lblNilai">Nilai Tugas 1</label><div class="row"><div class="col-sm-6" ><input type="text" class="form-control" name="nilai[]" placeholder="Masukkan Nilai (1-100)" value="'+dataArr[0].nilai+'"></div></div>';
		asu+='<input type="hidden" name="id[]" value="'+dataArr[0].id+'">';
		asu+='</div>';
		for(var i=1; i<dataArr.length; i++){
			asu+='<div class="form-group"><label class="lblNilai">Nilai Tugas '+(i+1)+'</label><div class="row"><div class="col-sm-6" ><input type="text" class="form-control" name="nilai[]" placeholder="Masukkan Nilai (1-100)" value="'+dataArr[i].nilai+'"></div><div class="col-sm-6"><a href="#" id="hapusForm2" data-toggle="tooltip" data-placement="right" title="Hapus Form"><i class="icon-remove"></i></a></div></div>';
		...
		if(dataArr.length>=1){
			...
			for(var i=0; i<dataArr.length; i++){
				table+='<th>Nilai Tugas '+(i+1).toString()+'</th>';
			}
			table+='</tr><tr>';
			for(var i=0; i<dataArr.length; i++){
				table+='<td>'+dataArr[i].nilai+'</td>';
				cok.eq(i).html('Nilai Tugas '+(i+1));
			}
			table+='</tr></table>';
		}else{
			table='<div class="alert alert-info" role="alert">Tidak ada record nilai. Nilai dimulai dari Tugas 1</div>';
			var start=1;
		}
...
$(function(){
	...
	$("#konfirmHapus").click(function(){
		...
		$.ajax({
			url:'<?php echo site_url("admin/nilai_tugas/hapusNilaiTugas"); ?>',
			type:'post',
			data:data,
			dataType:'json'
		}).done(function(data){
	...
	$("#modalTambahNilai [name='kd_kelas'],#modalSetting [name='kd_kelas']").change(function(e){
		...
		$.ajax({
			url:'<?php echo site_url('admin/nilai_tugas/getSiswaByKelas'); ?>',
			type:'get',
			...
			if(index==0){
				//alert(kelas);
				lbl='<option value="pilihsiswa">Pilih Siswa '+kelas+'</option>';
			}
			else lbl='';
			$(mbut).html(lbl+data);
	...
	$("#modalLog").on("hidden.bs.modal",function(e){
		if($("body").data("isRefresh")==true)window.location.reload();
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-table"></i> Nilai Tugas
                            </li>
                        </ol>
                    </div>
                </div>
				...
				<div class="col-lg-6">
				 <form class="form-inline" id="gantiPaging" method="post" action="<?php echo site_url("admin/nilai_tugas/gantiPaging"); ?>">
			...
			<form id="frmCari" method="get" action="<?php echo site_url("admin/nilai_tugas/index"); ?>">
			 <div class="col-lg-4">
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalTambahNilai">
  <div class="modal-dialog">
	...
	<form id="frmTambahNilai" method="post" action="<?php echo site_url("admin/nilai_tugas/tambahNilaiTugas"); ?>">
...
<option value="pilihsiswa">Pilih Siswa</option>
<?php echo $nisData; ?>
</select>
  </div>
  <hr>
...
<div id="nilaiForm">
<div class="form-group">
    <label class="lblNilai">Nilai Tugas 1</label>
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalEditNilai">
  <div class="modal-dialog">
    <div class="modal-content">
	<form id="frmeditNilai" method="post" action="<?php echo site_url("admin/nilai_tugas/editNilaiTugas"); ?>">
	...
	<input type="hidden" name="tahun_ajaranEdit" value="">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Edit Nilai Tugas</h4>
      </div>
      <div class="modal-body">
        <p>
  <div class="form-group">
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalSetting">
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
	<form method="post" action="<?php echo site_url("admin/nilai_tugas/gantiPaging"); ?>" >
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
